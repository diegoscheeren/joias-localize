...
export default {
    ...
    data: () => ({
        ...
        venda: '',
        excluirId: '',
        pageTitle: 'Selecionar Peças',
        backBtn: {show: true, to: '/cadastro-venda'},
    ...
    mounted () {
        this.venda = this.$store.getters.getData.venda;
        this.consultar(this.venda.itens);
    },
    beforeDestroy() {
        this.venda.itens = this.dados.filter(d => d.quantidade > 0)
        this.$store.commit('setData', {venda: this.venda});
    },
    methods: {
        consultar(addedItems = []) {
            ...
            const d = {colaborador: this.$store.getters.getUsuarioId};

            this.$http.put(this.$urlAPI + 'mostruario-itens-colaborador', d)
                .then(resp => {
                    this.loading = false;
